...
export const mockBackendInterceptor: HttpInterceptorFn = (request, next) => {
  ...
  if (method === 'POST' && path === '/ai/generate') {
    ...
    if (prompt === '') {
      return failValidation({ Prompt: ['Describe what you would like the assistant to create.'] });
    }
    if (prompt.length > 4000) {
      return failValidation({ Prompt: ['Keep the prompt under 4,000 characters.'] });
    }
    // Include the word "fail" in a prompt to see the error state.
    if (/\bfail\b/i.test(prompt)) {
      return fail(503, 'Service temporarily unavailable', 'The AI provider did not respond in time.', 'external_service_unavailable');
    }
    return ok({
      answer:
        `✨ Sample response (mock API)\n\n` +
        `Here's a message based on your request:\n"${prompt}"\n\n` +
        `Smile brighter this month! 😁 Book your visit today and enjoy 20% off selected treatments. ` +
        `Limited slots available, so reply to this message to reserve yours.\n\n` +
        `Connect the real API to get answers generated by AI.`,
    });
  }
